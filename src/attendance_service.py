...
def run_attendance_service(stop_event, caps, camera_indices, app_insight, host_ip, port):
    """
    ...
    while not stop_event.is_set():
        try:
            ...
            print(Fore.YELLOW +
                  f"Connection from {addr}. Starting Attendance Scan...")
            ...
            while (time.time() - scan_start_time < scan_duration):
                ...
                if len(caps) <= active_cam_idx:
                    print(
                        f"Error: Attendance Camera Index {active_cam_idx} out of range.")
                    time.sleep(1)
                    break
                ...
                if not ret:
                    ...
                    recovered_cap = handle_camera_failure(
                        caps[active_cam_idx], active_cam_idx, stop_event)
                ...
                for face in Faces:
                    ...
                    last_alert_time, AnomalyDetected, Labels = process_detected_face(
                        frame, face, None, last_alert_time, purpose='attendence',express_client=None
                    )
                    ...
                    if not AnomalyDetected:
                        ...
                        print(
                            Fore.GREEN + f"Valid User Found: {Labels} at {attendenceTime}")
                        ...
                        cv2.resizeWindow("Attendance Scan", 800, 600)
